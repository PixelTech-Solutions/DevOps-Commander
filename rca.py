...
_COORDINATOR_INSTRUCTIONS = (
    ...
    "PROVE IT WITH A READ-ONLY REMOTE CHECK — do not infer from VM power state "
    "alone. When the VM/instance is UP but telemetry is absent, run a read-only "
    "command on the host (Azure VM run-command or AWS SSM) to confirm the actual "
    "service state before concluding — e.g. 'systemctl status grafana-agent "
    "node-exporter' and 'journalctl -u grafana-agent --no-pager -n 50'. Base the "
    "root cause on what that command returned, not on a guess. Read-only "
    "diagnostics are safe and auto-approve; only the remediation (restart/start) "
    "is needs-human.\n"
